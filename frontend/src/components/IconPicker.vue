<template>
  <Teleport to="body">
    ...
    <!-- Bottom sheet -->
    <div
      class="fixed inset-x-0 bottom-0 z-50 flex max-h-[85vh] flex-col rounded-t-2xl bg-surface-white"
    ...
    >
      ...
      <div class="flex items-center justify-between px-4 py-2">
        <h2 class="text-base font-semibold text-ink-gray-9">Select Icon</h2>
        <button
          type="button"
      ...
      <div class="flex-1 overflow-y-auto px-4 pb-4">
        ...
        <template v-else-if="searchQuery.trim()">
          ...
          <div class="grid grid-cols-7 gap-1">
            ...
            >
              <span
                class="block h-7 w-7 text-ink-gray-8 [&>svg]:h-full [&>svg]:w-full"
        ...
        <template v-else>
          ...
          <div v-for="(names, group) in lucideIconGroups" :key="group" class="mb-5">
            <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-ink-gray-5">
              {{ group }}
            </p>
            <div class="grid grid-cols-7 gap-1">
              <button
